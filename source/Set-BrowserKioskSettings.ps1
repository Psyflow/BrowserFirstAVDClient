...
If ($AutoLogon) {
    If ($TriggerAction -eq 'Lock' -or $TriggerAction -eq 'Logoff') {Throw 'You cannot specify a TriggerAction of Lock or Logoff with AutoLogon'}
    Else {$TriggerAction = 'ResetClient'}

}
Else {
If ($TriggerAction -eq 'ResetClient') {Throw 'You cannot specify a TriggerAction of ResetClient without AutoLogon'}
...
# Ensures $SecurityKey is only enabled if a real vendor ID is present
If ($null -ne $DeviceVendorID -and $DeviceVendorID -ne '') {$SecurityKey = $true}
